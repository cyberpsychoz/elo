...
describe('toEloCode - DateTime', () => {
  ...
  it('serializes DateTime with time as datetime literal', () => {
    ...
    assert.ok(result.startsWith('D2024-01-15T'), `Expected datetime format, got: ${result}`);
  });
...
describe('toEloCode - arrays', () => {
  ...
  it('serializes nested arrays', () => {
    assert.strictEqual(toEloCode([[1, 2], [3, 4]]), '[[1, 2], [3, 4]]');
  });
...
describe('toEloCode - objects', () => {
  ...
  it('serializes object with multiple properties', () => {
    ...
    assert.ok(result.includes("name: 'Bob'"), `Expected name property, got: ${result}`);
    assert.ok(result.includes('age: 30'), `Expected age property, got: ${result}`);
  });
...
describe('toEloCode - functions', () => {
  it('serializes functions as <function>', () => {
    assert.strictEqual(toEloCode(() => 1), '<function>');
    assert.strictEqual(toEloCode(function test() { return 1; }), '<function>');
  });
});
