...
describe('eloAdapter', () => {
  it('serializes values to Elo code', () => {
    assert.strictEqual(eloAdapter.serialize({ a: 1, b: 'hello' }), "{a: 1, b: 'hello'}");
...
describe('getFormat', () => {
  ...
  it('throws for unknown format', () => {
    assert.throws(() => getFormat(defaultFormats, 'xlsx'), /Unknown format 'xlsx'/);
  });
  ...
  it('works with custom formats', () => {
    ...
    assert.deepStrictEqual(getFormat(formats, 'pipe').parse('a|b|c'), ['a', 'b', 'c']);
