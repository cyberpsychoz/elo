...
(raise "Assertion failed" unless (->(v) { case v when NilClass; 'Null' when ActiveSupport::Duration; 'Duration' when Date, DateTime, Time; 'DateTime' when Integer; 'Int' when Float; 'Float' when TrueClass, FalseClass; 'Bool' when String; 'String' when Proc; 'Fn' when Array; 'Array' else 'Object' end }).call(-1 * ActiveSupport::Duration.parse('P1D')) == "Duration"; true)
(raise "Assertion failed" unless Date.parse('2024-01-03') + -1 * ActiveSupport::Duration.parse('P1D') == Date.parse('2024-01-02'); true)
(raise "Assertion failed" unless (->(v) { case v when NilClass; 'Null' when ActiveSupport::Duration; 'Duration' when Date, DateTime, Time; 'DateTime' when Integer; 'Int' when Float; 'Float' when TrueClass, FalseClass; 'Bool' when String; 'String' when Proc; 'Fn' when Array; 'Array' else 'Object' end }).call((Date.parse('2024-01-15') - Date.parse('2024-01-10')).to_i.days) == "Duration"; true)
(raise "Assertion failed" unless (Date.parse('2024-01-15') - Date.parse('2024-01-10')).to_i.days == ActiveSupport::Duration.parse('P5D'); true)
