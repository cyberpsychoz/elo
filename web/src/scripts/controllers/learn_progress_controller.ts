...
export default class LearnProgressController extends Controller {
  ...
  private scrollToNextLesson(currentLessonId: string) {
    const currentIndex = this.lessonTargets.findIndex(el => el.id === currentLessonId);
  ...
  private updateLessonUI(lessonId: string) {
    ...
    const button = this.toggleBtnTargets.find(el => el.dataset.lesson === lessonId);
    if (button) {
    ...
    // Update TOC link
    const tocLink = this.tocLinkTargets.find(el => el.getAttribute('href') === `#${lessonId}`);
    if (tocLink) {
